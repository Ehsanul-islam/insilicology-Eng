-- ============================================================
-- DEMO COURSE SEED SCRIPT
-- Run this in Supabase Dashboard > SQL Editor
-- ============================================================

-- First, add the new columns if they don't exist (from migration 20251221000000)
DO $$ 
BEGIN
  -- Add promo_video_url column
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'promo_video_url') THEN
    ALTER TABLE public.courses ADD COLUMN promo_video_url TEXT;
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'instructor_photo') THEN
    ALTER TABLE public.courses ADD COLUMN instructor_photo TEXT;
  END IF;
  
  -- Add stats column
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'stats') THEN
    ALTER TABLE public.courses ADD COLUMN stats JSONB DEFAULT '{}';
  END IF;
  
  -- Add comparison_features column
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'comparison_features') THEN
    ALTER TABLE public.courses ADD COLUMN comparison_features JSONB DEFAULT '[]';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'target_audience') THEN
    ALTER TABLE public.courses ADD COLUMN target_audience JSONB DEFAULT '[]';
  END IF;
  
  -- Add value_breakdown column
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'value_breakdown') THEN
    ALTER TABLE public.courses ADD COLUMN value_breakdown JSONB DEFAULT '[]';
  END IF;
  
  -- Add countdown_end_date column
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'countdown_end_date') THEN
    ALTER TABLE public.courses ADD COLUMN countdown_end_date TIMESTAMPTZ;
  END IF;
  
  -- Add testimonials column
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'testimonials') THEN
    ALTER TABLE public.courses ADD COLUMN testimonials JSONB DEFAULT '[]';
  END IF;
  
  -- Add faq column
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'faq') THEN
    ALTER TABLE public.courses ADD COLUMN faq JSONB DEFAULT '[]';
  END IF;
  
  -- Add payment columns
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'payment_methods') THEN
    ALTER TABLE public.courses ADD COLUMN payment_methods JSONB DEFAULT '[]';
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'payment_instructions') THEN
    ALTER TABLE public.courses ADD COLUMN payment_instructions TEXT;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'courses' AND column_name = 'enrollment_form_fields') THEN
    ALTER TABLE public.courses ADD COLUMN enrollment_form_fields JSONB DEFAULT '[]';
  END IF;
END $$;
...
Join thousands of students who have transformed their careers through this bootcamp!',
  ...
  '[
    ...
    {"name": "Arif Hassan", "role": "Full Stack Developer", "video_url": "https://www.youtube.com/watch?v=dQw4w9WgXcQ", "text": "Sarah is an incredible instructor. She explains complex concepts in a way that is easy to understand.", "rating": 5},
  ...
  '[
    ...
    {"question": "Is there a money-back guarantee?", "answer": "Absolutely! We offer a 7-day money-back guarantee. If you are not satisfied with the course for any reason, just let us know within 7 days of enrollment and we will provide a full refund."},
...
SELECT id, slug, title, status FROM public.courses WHERE slug = 'complete-web-development-bootcamp';
