  ...
  try {
    ...
    switch (action) {
      case 'issue': {
        ...
        const { data: certificate, error: insertError } = await supabase
          ...
          .insert({
            ...
            is_active: true,
            download_enabled: false, // Pending admin approval by default
            download_count: 0,       // No downloads yet
            // verification_code will be auto-generated by database trigger
          })
          .select()
          .single()

        if (insertError) {
          console.error('CRITICAL INSERT ERROR:', JSON.stringify(insertError))
          return new Response(
      ...
      case 'enable_download': {
        ...
        if (updateError) {
          console.error('Error enabling download:', updateError)
          return new Response(
            JSON.stringify({ error: 'Failed to enable download' }),
            { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          )
        }

        console.log(`Download enabled for certificate: ${certificateId}`)
